...
#' 
#' @export
Ypop_sd <- function(ygroup, ps = c('true', 'estimated'), scores = NULL,
                 dta = NULL, use = 'everything',hajypop) {
  #testing##
  #hajypop = haj
  ...
  gamma <- as.numeric(dimnames(ygroup)[[3]])
  
  ypop <- hajypop
  
  ypop_var <- apply(ygroup, 3, function(x) cov(x, use = use))
  ypop_var <- array(ypop_var, dim = c(2, 2, length(gamma)))
  ...
  dimnames(ypop_var) <- list(a = c(0, 1), a = c(0, 1), gamma = gamma)
