...
BootVar_sd <- function(dta, B = 500, alpha, ps = c('true', 'est'), cov_cols,
  ...
  ygroup <- array(NA, dim = c(n_neigh, 2, num_gamma_cols, B))
  dimnames(ygroup) <- list(neigh = 1 : n_neigh, po = c('y0', 'y1'),
                           gamma = 1:num_gamma_cols, sample = 1 : B)
  
  boots <- array(NA, dim = c(2, num_gamma_cols, B))
  ...
  for (bb in 1 : B) {
    ...
    if (ps == 'true') { # Known propensity score.
      ...
      re_var_positive[bb] <- 0#(phi_hat_true[[2]] > 0)
      
      yhat_group_boot<- GroupIPW_sd2(dta = boot_dta, cov_cols = cov_cols_gamma, phi_hat = 0,
                                     alpha = alpha, trt_col = trt_col, out_col = out_col,
                                     estimand = estimand, alpha_re_bound = 15, verbose = F,
                                     gamma_numer = gamma_numer, loud_denom = F, fix_phi = F)#$yhat_group
      
      z <- yhat_group_boot$yhat_group 
      po = apply(z, c(2,3), mean)
      
      #hajek adj
      #sum the weights for each cluster
      wts = yhat_group_boot$wt_list #the sum of this over cluster is n_hz
      ...
      size = boot_dta %>% group_by(neigh) %>% summarise(cluster_pop = n())
      yhat_group_boot <- yhat_group_boot$yhat_group 
      
      #get the sum of the potential outcomes in each cluster (rather than the mean over individual units)
      test = apply(yhat_group_boot, c(2,3), "*", size$cluster_pop) #this is yhat * num units
      test = apply(test, c(2,3), sum) #this is summed over all clusters
      #print(test)
      
      #print out the test / wts (shoudl be the estiamdn with hajek weight)
      haj = test / wts
      
      ygroup[, , , bb] <- yhat_group_boot
      boots[, , bb] <- haj
